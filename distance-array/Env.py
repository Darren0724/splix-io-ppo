...
class GridToImageWrapper(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        if 'grid' not in env.observation_space.spaces or 'distance_array' not in env.observation_space.spaces:
            raise ValueError("The wrapped environment's observation_space Dict must contain 'grid' and 'distance_array' keys.")

        self.grid_space = env.observation_space.spaces['grid'] # Shape (H, W, C_one_hot)
        self.distance_array_space = env.observation_space.spaces['distance_array']
        self.last_action_space = env.observation_space.spaces['last_action'] 
        ...
        self.observation_space = spaces.Dict({
            ...
            'last_action': self.last_action_space,
            'distance_array': self.distance_array_space
        })
    ...
    def observation(self, obs_dict):
        ...
        return {'grid': processed_grid, 
                'distance_array': obs_dict['distance_array'], # Already correct format
                'last_action': obs_dict['last_action'].astype(np.float32)} # Ensure last_action is float
...
class SplixIOEnv(gym.Env):
    ...
    def __init__(self, grid_size=ACTUAL_GRID_SIZE, obs_size=OBS_SIZE, render_mode='human'):
        ...
        self.observation_space = spaces.Dict({
            ...
            'distance_array': spaces.Box(low=0, high=1, shape=(4 * self.obs_size,), dtype=np.int32),
            'last_action': spaces.Box(low=0, high=1, shape=(NUM_ACTIONS,), dtype=np.int32) # One-hot encoded action
    ...
    def gen_obs(self):
        ...
        # also add four distance arrays to the observation
        distance_array = np.zeros((4, self.obs_size), dtype=np.float32)  # Four directions: up, down, left, right
        for i in range(self.obs_size):
            head = self.real_grid[center_r, center_c]
            # Up direction
            if 0 <= center_r - half_obs + i < self.actual_grid_size:
                j = 0
                while center_c - j >= 0 and self.real_grid[center_r - half_obs + i, center_c - j] == head:
                    j += 1
                distance_array[0, i] = j / self.actual_grid_size if center_c - j >= 0 else 1
            else:
                distance_array[0, i] = 1
            # Down direction
            if 0 <= center_r + half_obs + i < self.actual_grid_size:
                j = 0
                while center_c - j >= 0 and self.real_grid[center_r + half_obs + i, center_c - j] == head:
                    j += 1
                distance_array[1, i] = j / self.actual_grid_size if center_c - j >= 0 else 1
            else:
                distance_array[1, i] = 1
            # Left direction
            if 0 <= center_c - half_obs + i < self.actual_grid_size:
                j = 0
                while center_r - j >= 0 and self.real_grid[center_r - j, center_c - half_obs + i] == head:
                    j += 1
                distance_array[2, i] = j / self.actual_grid_size if center_r - j >= 0 else 1
            else:
                distance_array[2, i] = 1
            # Right direction
            if 0 <= center_c + half_obs + i < self.actual_grid_size:
                j = 0
                while center_r + j < self.actual_grid_size and self.real_grid[center_r + j, center_c + half_obs + i] == head:
                    j += 1
                distance_array[3, i] = j / self.actual_grid_size if center_r + j < self.actual_grid_size else 1
            else:
                distance_array[3, i] = 1
        distance_array = distance_array.flatten()  # Flatten to 1D array of shape (4 * obs_size,)

        return {"grid": obs_grid_one_hot, "distance_array": distance_array,"last_action": self.last_action} # Return as a dict
    def render(self):
        if self.render_mode == 'human':
