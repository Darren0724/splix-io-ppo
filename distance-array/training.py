...
import logging
# Set up logging
logging.basicConfig(filename='ppo_splix_io_partial_obs.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S')
# Check if running on Windows
if platform.system() == 'Windows':
    # Use asyncio's ProactorEventLoop on Windows
...
def plot_reward_curves(train_history, eval_history, save_dir):
    if not train_history and not eval_history:
        logging.info("No reward history to plot.")
        return
    ...
    if train_history:
        ...
        logging.info(f"Training reward curve saved to {os.path.join(save_dir, 'train_reward.png')}")
    if eval_history:
        ...
        logging.info(f"Evaluation reward curve saved to {os.path.join(save_dir, 'eval_reward.png')}")
...
async def train_and_evaluate(total_timesteps=100000, eval_episodes_final=10, load_model_path=None,
    ...
    env_kwargs_final_eval = {'grid_size': ACTUAL_GRID_SIZE, 'obs_size': OBS_SIZE, 'render_mode': 'human' if save_video_path else 'none'}
    ...
    if load_model_path and os.path.exists(load_model_path + ".zip"):
        logging.info(f"Loading pre-trained model from {load_model_path}.zip ...")
        ...
        if os.path.exists(env_stats_path_to_load):
            logging.info(f"Loading environment stats from {env_stats_path_to_load}...")
            ...
            logging.info("Environment stats loaded into current VecNormalize wrapper.")
        else:
            logging.info(f"Warning: Environment stats file not found at {env_stats_path_to_load}. Using fresh stats for loaded model.")
        env.training = True
        model.set_env(env) # Ensure the potentially updated env is set
    else:
        if load_model_path: logging.info(f"Warning: Model file not found at {load_model_path}.zip. Initializing new model.")
    ...
    if not load_model_flag:
        ...
        logging.info(f"Saving final model to {FINAL_MODEL_SAVE_PATH}.zip and environment stats to {ENV_STATS_SAVE_PATH}...")
        model.save(FINAL_MODEL_SAVE_PATH)
        env.save(ENV_STATS_SAVE_PATH) # Save stats of the current env
    else:
        logging.info("Skipping training as load_model_flag is True.")
    ...
    if os.path.exists(best_model_path): # Prioritize best model if training occurred
        logging.info(f"Loading best saved model from {best_model_path} for final evaluation...")
        model_to_eval = PPO.load(best_model_path, custom_objects={'policy_kwargs': policy_kwargs})
         ...
         logging.info(f"Using initially loaded model for final evaluation: {load_model_path}.zip")
         # model_to_eval is already the loaded model
    elif not os.path.exists(best_model_path) and load_model_flag and load_model_path:
        logging.info(f"Initially loaded model specified but not found at {load_model_path}.zip. Using current model if available.")
    else:
        logging.info("Using the final/current model for evaluation.")
    ...
    if eval_episodes_final > 0 and model_to_eval is not None:
        ...
        try:
            ...
            for episode_idx in tqdm(range(eval_episodes_final), desc="Final Evaluation", unit="episode"):
                ...
                if save_video_path and eval_env_wrapped_final.render_mode == 'human':
                    try:
                        ...
                        logging.info(f"Could not initialize video writer for episode {episode_idx}: {e}.")
                    ...
                    if eval_env_wrapped_final.render_mode == 'human':
                        ...
                        for event in pygame.event.get(): 
                            if event.type == pygame.QUIT:
                                logging.info("Pygame window closed during final evaluation.")
                ...
                if video_writer_episode:
                    ...
                    except Exception as e: logging.info(f"Error closing video writer for ep {episode_idx}: {e}")
                video_writer_global = None 
            ...
            if total_rewards:
                ...
                logging.info(f"Average steps over {len(total_steps)} final episodes: {np.mean(total_steps):.2f}")
            else: logging.info("No final evaluation episodes were completed.")
        finally:
            if video_writer_global: 
                try: video_writer_global.close()
                except Exception as e: logging.info(f"Error closing global video writer in finally: {e}")
            if 'eval_env_wrapped_final' in locals() and eval_env_wrapped_final: eval_env_wrapped_final.close()
    elif model_to_eval is None:
        logging.info("No model available for final evaluation.")
...
if __name__ == "__main__":
    ...
    EVAL_EPISODES_FINAL_VISUAL_MAIN = 20
