...
class GridToImageWrapper(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        if 'grid' not in env.observation_space.spaces or 'sign_array' not in env.observation_space.spaces:
        ...
        self.sign_array_space = env.observation_space.spaces['sign_array']
        ...
        self.observation_space = spaces.Dict({
            ...
            'sign_array': self.sign_array_space
        })
    ...
    def observation(self, obs_dict):
        ...
        return {'grid': processed_grid, 
                'sign_array': obs_dict['sign_array'], # Already correct format
...
class SplixIOEnv(gym.Env):
    ...
    def __init__(self, grid_size=ACTUAL_GRID_SIZE, obs_size=OBS_SIZE, render_mode='human'):
        ...
        self.observation_space = spaces.Dict({
            ...
            'sign_array': spaces.Box(low=0, high=1, shape=(8,), dtype=np.int32),
    ...
    def gen_obs(self):
        ...
        # also add a array of eight element boolean indicates that the ninth direction exist a EMPTY_ID or not
        sign_array = np.zeros((8,), dtype=np.int32) # 8 directions around the player
        cnt = 0
        for i in range(-1,2):
            for j in range(-1,2):
                if i == 0 and j == 0: continue
                # Check if cnt is within bounds of sign_array before assignment
                if cnt < sign_array.shape[0]: 
                    for t in range(1,self.actual_grid_size):
                        r = center_r + i * t
                        c = center_c + j * t
                        if 0 <= r < self.actual_grid_size and 0 <= c < self.actual_grid_size:
                            if self.grid[r, c] == EMPTY_ID:
                                sign_array[cnt] = 1
                                break
                            elif self.grid[r,c] != TERRITORY_ID:
                                # If we hit a non-territory cell, we stop checking this direction
                                break
                        else:
                            break # Out of bounds, stop checking this direction
                cnt += 1 # Increment cnt for each of the 8 directions

        return {"grid": obs_grid_one_hot, "sign_array": sign_array,"last_action": self.last_action} # Return as a dict
    def render(self):
        if self.render_mode == 'human':
