...
class CustomCNN(BaseFeaturesExtractor):
    def __init__(self, observation_space: spaces.Dict, features_dim: int = HIDDEN_DIM_DEFAULT):
        ...
        if 'grid' not in observation_space.spaces or \
           'sign_array' not in observation_space.spaces or \
        ...
        sign_array_space = observation_space.spaces['sign_array']
        ...
        n_flatten = n_flatten_cnn + sign_array_space.shape[0] + last_action_space.shape[0]
    ...
    def forward(self, observations: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        sign_array_input = observations['sign_array']
        ...
        sign_array_input = sign_array_input.float()
        last_action_input = last_action_input.float()

        x_combined = torch.cat((x_cnn_flattened, sign_array_input, last_action_input), dim=1)
